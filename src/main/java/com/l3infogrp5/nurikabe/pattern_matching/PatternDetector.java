...
/**
 * Classe permettant de détecter un pattern dans une grille
 */
public class PatternDetector {
  ...
  public PatternDetector(int[][] pattern) {
    ...
    {
      patternHash.add(ZHash.hash(rotatedPattern, patternRowSize, patternColSize));
    }
  ...
  public ArrayList<int[]> detectInGrid(int[][] grid) {
    ...
    // Pour chaque sous-grille de la taille du pattern dans la grille, nous vérifions si leur hash correspond à celui du pattern
    for (int i = 0; i <= gridRowSize - patternRowSize; i++) {
      for (int j = 0; j <= gridColSize - patternColSize; j++) {
        ...
        if (index > -1) {
          // Si oui, ajouter la position (i, j) à la liste des positions du pattern dans la grille
